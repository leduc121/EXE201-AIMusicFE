...
import { DEMO_SONG } from '../data/MockSong';

export function PianoLearningPage() {
  ...
  useEffect(() => {
    ...
    if (isPlaying) {
      const playSong = () => {
        DEMO_SONG.forEach((event) => {
          const t1 = setTimeout(() => {
            setActiveKeys([event.note]);
    ...
    <div className="min-h-screen w-full bg-[#2a1b15] flex flex-col">
      ...
      <div className="flex-1 bg-[#1a100c] relative flex flex-col">
        <div className="w-full max-w-6xl mx-auto pt-8 px-4 flex-1 flex flex-col justify-center">
          <SheetMusic isPlaying={isPlaying} currentNote={activeKeys[0]} />
      ...
      {/* Piano Area */}
      <div className="relative h-[500px] border-4 border-[#3E2723] rounded-lg overflow-hidden shadow-2xl">
        <Scene3D activeKeys={activeKeys} />
      </div>
    </div>);
