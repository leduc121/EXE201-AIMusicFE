...
export function PricingPlans({ onNavigate }: { onNavigate?: (page: string) => void }) {
  return (
          ...
          {PLANS.map((plan) => (
            ...
            >
              ...
              <div className="flex justify-between items-center mb-6">
                <h3 className={`text-2xl font-bold font-serif ${plan.id === 'pro' ? 'text-[#FAF7F0]' : plan.id === 'studio' ? 'text-[#D4A574]' : 'text-[#8B4513]'
                  }`}>
                  {plan.name}
                </h3>
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="flex items-end gap-1">
                  <span className={`text-4xl font-bold ${plan.id === 'starter' ? 'text-[#3E2723]' : 'text-white'
                    }`}>
                    {plan.price}
                  </span>
                  <span className={`text-sm mb-1 ${plan.id === 'pro' ? 'text-[#FAF7F0]/70' : plan.id === 'studio' ? 'text-[#FAF7F0]/60' : 'text-[#654321]'
                    }`}>
                    {plan.billing}
                  </span>
                </div>
              ...
              <Button
                ...
                className={`w-full py-6 text-lg mb-8 transition-all ${plan.id === 'pro'
                ...
                <ul className="space-y-3">
                  {plan.features.map((feature, index) => (
                    <li key={index} className="flex items-start gap-3">
                      <Check
                        className={`w-5 h-5 shrink-0 ${plan.id === 'starter' ? 'text-[#8B4513]' : 'text-[#D4A574]'
                          }`}
                      />
                      <span className={`text-sm ${plan.id === 'pro' ? 'text-[#FAF7F0]/90' : plan.id === 'studio' ? 'text-[#FAF7F0]/80' : 'text-[#3E2723]/90'
                        } font-serif`}>
                        {feature}
                      </span>
                    </li>
